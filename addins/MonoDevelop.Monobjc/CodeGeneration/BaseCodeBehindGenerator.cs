...
namespace MonoDevelop.Monobjc.CodeGeneration
{
    ...
    public abstract class BaseCodeBehindGenerator : ICodeBehindGenerator
    {
        ...
        public FilePath GenerateDesignCodeForFrameworks(ProjectResolver resolver, String[] frameworks)
        {
        ...
        /// <returns>The path to the designer file.</returns>
        public FilePath GenerateDesignCodeForIB(ProjectResolver resolver, CodeBehindWriter writer, String className, IEnumerable<IBPartialClassDescription> enumerable)
        {
        ...
        protected virtual CodeTypeMember GenerateOutletProperty(IType outletType, string name)
        {
            ...
            property.CustomAttributes.Add(
                new CodeAttributeDeclaration("IBOutlet"));
            property.CustomAttributes.Add(
                new CodeAttributeDeclaration("ObjectiveCIVar",
                                             new CodeAttributeArgument(new CodePrimitiveExpression(name))));

            // Generate the getter
            property.GetStatements.Add(
                new CodeMethodReturnStatement(
                    new CodeMethodInvokeExpression(getterRef, nameRef)));

            // Generate the setter
            property.SetStatements.Add(
                new CodeMethodInvokeExpression(setterRef, nameRef, valueRef));
        ...
        protected virtual CodeTypeMember GenerateActionExposedMethod(String message, IType argumentType)
        {
            ...
            method.CustomAttributes.Add(
                new CodeAttributeDeclaration("IBAction"));
            method.CustomAttributes.Add(
                new CodeAttributeDeclaration("ObjectiveCMessage",
                                             new CodeAttributeArgument(selectorRef)));

            // Add body to call partial method
            method.Statements.Add(
                new CodeMethodInvokeExpression(thisRef, name, senderRef));
