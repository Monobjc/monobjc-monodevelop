    ...
    public class ProjectResolver
    {
        ...
        public IEnumerable<IType> GetAllClasses(bool projectOnly)
        {
            Func<ProjectDom, IEnumerable<IType>> matcher = dom => from t in dom.Types
                                                                  where t.ClassType == ClassType.Class &&
                                                                        t.Attributes != null &&
                                                                        t.Attributes.Any(a => String.Equals(OBJECTIVECCLASSATTRIBUTE, a.AttributeType.FullName))
                                                                  select t;
        ...
        public IEnumerable<IType> GetAllProtocols(bool projectOnly)
        {
            Func<ProjectDom, IEnumerable<IType>> matcher = dom => from t in dom.Types
                                                                  where t.ClassType == ClassType.Interface &&
                                                                        t.Attributes != null &&
                                                                        t.Attributes.Any(a => String.Equals(OBJECTIVECPROTOCOLATTRIBUTE, a.AttributeType.FullName))
                                                                  select t;
            return GetMatchingTypes(matcher, projectOnly);
        }
